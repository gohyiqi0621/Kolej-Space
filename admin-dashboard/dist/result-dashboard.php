...
$sql = "SELECT semester_id, pointer FROM student_results WHERE matric_no = ? ORDER BY semester_id ASC";
...
while ($row = $result->fetch_assoc()) {
    $semesters[$row['semester_id']][] = $row['pointer'];
}
                                    ...
                                    <div class="table-responsive">
                                        ...
                                        <?php else: ?>
                                            <table class="table table-centered m-0">
                                                ...
                                                <tbody>
                                                    <?php foreach ($semesters as $semId => $pointers): 
                                                        $semTotal = array_sum($pointers);
                                                        $semCount = count($pointers);
                                                        $gpa = $semCount > 0 ? $semTotal / $semCount : 0;

                                                        $totalPoints += $semTotal;
                                                        $totalSubjects += $semCount;
                                                        $cgpa = $totalSubjects > 0 ? $totalPoints / $totalSubjects : 0;

                                                        $status = $gpa >= 2.0 ? 'Pass' : 'Failed';
                                                        $badgeClass = $gpa >= 2.0 ? 'success' : 'danger';
                                                    ?>
                                                    <tr>
                                                        <td><a href="result-semester.php?semester_id=<?= urlencode($semId) ?>">Semester <?= htmlspecialchars($semId) ?></a></td>
                                                        <td><?= number_format($gpa, 2) ?></td>
                                                        <td><?= number_format($cgpa, 2) ?></td>
                                                        <td><span class="badge badge-<?= htmlspecialchars($badgeClass) ?>"><?= htmlspecialchars($status) ?></span></td>
                                                    </tr>
                                                    <?php endforeach; ?>
                                                </tbody>
                                            </table>
