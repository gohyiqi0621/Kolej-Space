...
error_log("DEBUG: reset-password.php started");
session_start();
...
// Check if user is authorized (email session variable exists from OTP verification)
if (!isset($_SESSION['email'])) {
    error_log("Unauthorized access attempt to reset-password.php");
    $basePath = dirname($_SERVER['PHP_SELF']);
    header("Location: $basePath/forget_password.php");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        $stmt = $conn->prepare("UPDATE users SET password = ?, otp_code = NULL, otp_expiry = NULL WHERE email_address = ?");
        if (!$stmt) {
            throw new Exception("Prepare failed: " . $conn->error);
        }
        ...
        if ($affected > 0) {
            ...
            $basePath = dirname($_SERVER['PHP_SELF']);
            header("Location: $basePath/sign-in.html");
            exit;
        } else {
            throw new Exception("Failed to update password. Email may not exist.");
        }
    } catch (Exception $e) {
        $error = $e->getMessage();
...
$conn->close();
error_log("DEBUG: reset-password.php reached end of file");
?>
                    ...
                    <div class="col-xl-8">
                        <div class="sign-in-items">
                            <div class="title text-center">
                                <h2 class="wow fadeInUp">Reset Password</h2>
                            </div>
                            <form action="reset-password.php" id="reset-password-form" method="POST">
                                <div class="error-message text-danger text-center mt-2"><?php echo isset($error) ? $error : ''; ?></div>
                                <div class="success-message text-success text-center mt-2"><?php echo isset($success) ? $success : ''; ?></div>
                                <div class="row g-4">
                                    <div class="col-lg-12">
                                        <div class="form-clt style-2">
                                            <span>Email Address</span>
                                            <input type="email" name="email" id="email" value="<?php echo isset($_SESSION['email']) ? htmlspecialchars($_SESSION['email']) : ''; ?>" readonly>
                                        </div>
                                    </div>
                                    <div class="col-lg-12">
                                        <div class="form-clt">
...
<script>
document.querySelectorAll('.form-clt .icon i').forEach(icon => {
    icon.addEventListener('click', () => {
        const input = icon.closest('.form-clt').querySelector('input');
